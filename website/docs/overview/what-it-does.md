...
**Figure 2: the issuer panel and the verifier panel side by side, right after a LIVE proof**

![Two dark panels: on the left the issuer panel with a TRUSTED issuer and one ACTIVE attestation; on the right the verifier panel showing a LIVE stamp, the public issuer id and validity window, and a black bar where the compliance file would be](/img/screens/both-live.png)

*Source: the authors (2026).*

</div>

Figure 2 is the whole product on one screen: the issuer keeps the record, the verifier gets a status and the public fields, and the place where the file would be is a labelled black bar.
...
Flowchart 1 shows which data stays with the issuer, which travels once in the
proof packet, and which is public on the Midnight ledger.

<div className="at-figure">

**Flowchart 1: the private, off-chain and public sides of an attestation**

```mermaid
flowchart LR
  subgraph PRIV["Issuer's machine (private)"]
    D["Raw compliance document"]
    S["issuerSecret"]
    R["revocationSecret"]
    REC["Attestation record + salt"]
  end
  subgraph PKT["Proof packet (off-chain, handed over once)"]
    P["rawDataHash, validFrom, validUntil,<br/>issuerId, nullifierHash, salt, commitment<br/>(no secrets, no document)"]
  end
  subgraph PUB["Midnight ledger (public)"]
    T["attestations tree:<br/>commitment leaves and root"]
    N["revokedNullifiers:<br/>revoked nullifierHash entries"]
    L["trustedIssuers:<br/>SIMULATED trust list flags"]
    V["Each proveLive call discloses status, issuerId,<br/>validFrom/validUntil and reads nullifierHash"]
  end
  D -->|"hashed into rawDataHash"| REC
  S -->|"hashed into issuerId"| REC
  R -->|"hashed into nullifierHash"| REC
  REC -->|"registerAttestation: commitment only"| T
  REC -->|"Export proof packet"| P
  R -->|"revokeAttestation: nullifierHash"| N
  P -->|"verifier imports it, runs proveLive"| V
  T --> V
  N --> V
  L --> V
```
...
[Architecture](/architecture) for the component diagram (Flowchart 3).
