...
Screens and GIFs on this site are the real `bboard-ui` running against a local devnet, recorded by the team with a scripted browser. In those recordings a small wallet bridge signs and submits each transaction with a real wallet instead of the 1AM popup, so no approval dialog appears; proof-generation waits in the GIFs are sped up. Figures 6 to 14 follow the steps above in order.

<div className="at-figure">

**Figure 6: the start screen, one connection slot per institution**

![Two cards: Connect the issuers wallet, with Deploy new demo registry and Join existing contract buttons, and Connect the verifiers wallet, with a Connect and join button](/img/screens/connect.png)
...
**Figure 7: the issuer panel after adding itself to the SIMULATED TRUST LIST**

![The issuer panel with the SIMULATED TRUST LIST badge, the issuer id, the gold attestations root, a green TRUSTED chip, and the demo attestation form with the SIMULATED SANCTIONS LIST badge](/img/screens/issuer-trusted.png)
...
**Figure 8: registering a sanctions-screening attestation, which appears as ACTIVE once the transaction lands**

![The issuer selects Sanctions screening, presses Register attestation, and the Registered list gains one ACTIVE entry](/img/gifs/register.gif)
...
**Figure 9: the proof packet the issuer exports once: hashes and a validity window, no secret and no file**

![A dialog titled Proof packet showing JSON with rawDataHash, validFrom, validUntil, issuerId, nullifierHash, salt and commitment, and Copy to clipboard and Close buttons](/img/screens/proof-packet.png)
...
**Figure 10: the verifier panel after importing the packet, before any proof is requested**

![The verifier panel with its own connected wallet, the imported attestation selected, a Request proof button and a Not verified yet badge](/img/screens/verifier-imported.png)
...
**Figure 11: the verifier panel after an on-chain proveLive: LIVE, public fields, and the redacted file**

![The verifier panel with a green LIVE stamp, the last on-chain proof time, issuer id, valid from and valid until, a black bar for the compliance file, and Approve and Reject buttons](/img/screens/verifier-live.png)
...
**Figure 12: the issuer revokes the attestation**

![The issuer presses Revoke and the attestation chip turns from ACTIVE to REVOKED](/img/gifs/issuer-revoke.gif)
...
**Figure 13: the verifier panel after proving again: REVOKED, and Approve disabled**

![The verifier panel with a red REVOKED stamp, a newer last on-chain proof time, and the Approve button disabled](/img/screens/verifier-revoked.png)
...
**Figure 14: both panels at the end of the cycle**

![The issuer panel with the attestation marked REVOKED next to the verifier panel showing the REVOKED stamp](/img/screens/both-revoked.png)
