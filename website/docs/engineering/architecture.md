...
Flowchart 3 shows every component and how data moves between them.

<div className="at-figure">

**Flowchart 3: components and data flow across issuer, contract, indexer and verifier**
...
flowchart LR
    ...
    subgraph Verifier["Verifier institution: separate browser session, separate private state"]
        VerifierPanel["Verifier panel\nimport packet / prove locally, submit proveLive tx / approve-reject"]
    end
    ...
    VerifierPanel -- "5. prove locally, submit proveLive tx (verifier pays DUST)" --> ProveLive
    ProveLive --> Kernel
    ProveLive --> Disclose
...
Flowchart 3 shows components and data flow; Flowchart 4 shows the order events actually
happen in, across two independent identities and one ledger neither of them controls
alone:

<div className="at-figure">

**Flowchart 4: the register, prove and revoke cycle in time order**
...
sequenceDiagram
    ...
    Ix-->>V: path
    V->>L: proveLive(ref): proof built locally from the packet + path, submitted as a tx the verifier pays DUST for
...
The two calls to `proveLive` in Flowchart 4 are the same call, run twice, by the same
...
Flowchart 5 shows the two public structures side by side.

<div className="at-figure">

**Flowchart 5: revocation writes to the nullifier map, not the tree**
...
Table 13 lists each component of the system.

<div className="at-figure">

**Table 13: components, what each owns and why it exists**
