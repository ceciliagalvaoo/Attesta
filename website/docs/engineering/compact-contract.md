...
Flowchart 6 shows the order in which `proveLive` evaluates these checks, as
written in the contract: revocation is checked first, then the validity window, then the
trust list.

<div className="at-figure">

**Flowchart 6: how proveLive decides the status**

```mermaid
flowchart TD
  A["proveLive(ref): record built from the<br/>imported packet fields"] --> B["Recompute persistentCommit(record, salt)<br/>and fetch the Merkle path"]
  B --> C{"path.leaf equals commitment<br/>and checkRoot(root) passes?"}
  C -->|"no"| X["Assertion fails:<br/>transaction rejected, no status"]
  C -->|"yes"| D{"nullifierHash is a member<br/>of revokedNullifiers?"}
  D -->|"yes"| R["REVOKED"]
  D -->|"no"| E{"Block time at or after validFrom<br/>and before validUntil?"}
  E -->|"no"| EX["EXPIRED"]
  E -->|"yes"| F{"issuerId is a member<br/>of trustedIssuers?"}
  F -->|"no"| NT["NOT_TRUSTED"]
  F -->|"yes"| L["LIVE"]
  R --> O["disclose(status, issuerId)"]
  EX --> O
  NT --> O
  L --> O
```
...
Flowchart 7 contrasts the two ways a record is built.

<div className="at-figure">

**Flowchart 7: write path and read path for issuerId and nullifierHash**

```mermaid
flowchart LR
  subgraph WR["Write path: registerAttestation, revokeAttestation"]
    S1["issuerSecret()"] -->|"persistentHash"| RO["recordOf(ref)"]
    S2["revocationSecret(ref)"] -->|"persistentHash"| RO
  end
  subgraph RD["Read path: proveLive"]
    W1["issuerIdWitness(ref)"] --> RP["recordForProveLive(ref)"]
    W2["nullifierHashWitness(ref)"] --> RP
  end
  RO --> C1["persistentCommit(record, salt)"]
  RP --> C2["persistentCommit(record, salt)"]
  C2 --> M["Must match a real leaf of attestations,<br/>otherwise the proof fails closed"]
```
